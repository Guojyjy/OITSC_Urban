...
if __name__ == '__main__':
    ...
    env = UrbanEnvImportantObservation(net_file=ABS_DIR + 'nets/1km/selected1km.net.xml',
                                       ...
                                       additional_sumo_cmd=['--tripinfo-output',
                                                            'ImportantObservation-SARSA-tripinfo-baseline-run{}-iter'.format(
                                                                runs)],
                                       out_csv_name='ImportantObservation-SARSA-output-baseline-run{}'.format(runs),
                                       oitsc=False
                                       )
    ...
    for run in range(1, runs + 2):
        ...
        outfile = open('ImportantObservation-SARSA-evaluation-baseline_run{}_iter{}.csv'.format(runs, run), 'w')
