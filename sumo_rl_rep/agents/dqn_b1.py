...
from sumo_rl_rep.util.HistoryNamespace import HistoryNamespace
from sumo_rl_rep.util.data_processing import PrettyFloat, Dict
...
class DQN(OffPolicyRLModel):
    ...
    def learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="DQN",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            waiting_time_data = []  # TODO: structure? data for output
            ...
            for _ in range(total_timesteps):
                ...
                for agent_id in range(0, 16):
                    # TODO: agent_id should be customised name/index of traffic light controller
                ...
                # TODO: history... structure for tl? time sequence

                neighbors_list, time_on_phase_list, phase_id_list, step_num, veh_position_data, veh_waiting_time, \
                veh_complete_data, total_new_obs, total_rew, done, info = self.env.step(env_action)
                # TODO：link to env，obtain traffic information, more than needed?
                ...
                if step_num == 1:
                    waiting_time_entry = "step_num,p_num,vehicle_name,type_name,position,lane_num,waiting_time".split(
                        ",")
                    # TODO: info set for each veh, p_num?
                    waiting_time_data.append(waiting_time_entry)
                    # TODO: list:1
                    #  [['step_num', 'p_num', 'vehicle_name', 'type_name', 'position', 'lane_num', 'waiting_time']]
                        ...
                        for cc in sorted(veh_complete_data[aa][bb].keys()):
                            for dd in sorted(veh_complete_data[aa][bb][cc].keys()):
                                for ee in sorted(veh_complete_data[aa][bb][cc][dd].keys()):
                                    # print (step_num,",", aa,",",bb,",",cc,",",dd,",",ee,","
                                    # ,PrettyFloat(veh_complete_data[aa][bb][cc][dd][ee]))
                                    intuitive_waiting_time = 0.0
                                    if step_num != 1 and veh_waiting_time_pre.get(aa, {}).get(bb, {}) != {}:
                                        if dd <= veh_position_data_pre[aa][bb] + 10:
                                            # TODO：+ 10? only move within 10m > stationary vehicles
                                            if phase_id_list[aa] == phase_id_pre_list[aa]:
                                                intuitive_waiting_time = veh_waiting_time_pre[aa][bb] + (
                                                        time_on_phase_list[aa] - time_on_phase_pre_list[aa])
                                                # TODO: this is updated and accumulative waiting time
                                            else:
                                                intuitive_waiting_time = veh_waiting_time_pre[aa][bb] + \
                                                                         time_on_phase_list[aa]
                                                # TODO: time_on_phase_list for signal phase switching
                                        else:
                                            intuitive_waiting_time = veh_waiting_time_pre[aa][bb]
                                    # e.g., ee = "3to7", 7 - 3 != 1
                                    if minute_time % 2 == 0 and (int(ee[ee.index(s2) + len(s2):]) - int(ee[:ee.index(s2)
                                       + len(s2)].replace('to', ''))) == 1 and random.randint(0, 4) == 2:
                                        total_rew[aa] = total_rew[aa] + veh_complete_data[aa][bb][cc][dd][
                                            ee]  # - intuitive_waiting_time
                                        # veh_waiting_time[aa][bb] = intuitive_waiting_time
                                        # TODO: diff from dqn_p1
                                    if minute_time % 2 == 0 and (int(ee[ee.index(s2) + len(s2):]) - int(ee[:ee.index(s2)
                                       + len(s2)].replace('to', ''))) != 1 and random.randint(0, 4) == 2:
                                        total_rew[aa] = total_rew[aa] + veh_complete_data[aa][bb][cc][dd][
                                            ee]  # - intuitive_waiting_time
                                        # veh_waiting_time[aa][bb] = intuitive_waiting_time
                                        # TODO: diff from dqn_p1
                                        # print("total_rew[aa]",total_rew[aa])
                                        # print("veh_complete_data[aa][bb][cc][dd][ee]",aa, bb, veh_complete_data[aa][bb][cc][dd][ee])
                                        # print("intuitive_waiting_time",intuitive_waiting_time)
                                    waiting_time_entry = step_num, aa, bb, cc, dd, ee, PrettyFloat(
                                        veh_complete_data[aa][bb][cc][dd][ee])
                                    # TODO: aa = p_num, tl_id
                                    #  bb = vehicle name,
                                    #  cc = type name,
                                    #  dd = position,
                ...
                for agent_id in range(0, 16):
                    # TODO: agent_id should be customised name/index of traffic light controller
                    new_obs = total_new_obs[str(agent_id)]
                    rew = total_rew[str(agent_id)]
                    # TODO: aim to update the reward?
            ...
            outfile = open('DQN/ImplicitObservation/Implicit-Observation-DQN-result-BASELINE-run1.csv', 'w')
