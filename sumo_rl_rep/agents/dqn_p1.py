    ...
    def learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="DQN",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for _ in range(total_timesteps):
                ...
                neighbors_list, time_on_phase_list, phase_id_list, step_num, veh_position_data, veh_waiting_time, veh_complete_data, total_new_obs, total_rew, done, info = self.env.step(
                    env_action)
                                ...
                                for ee in sorted(veh_complete_data[aa][bb][cc][dd].keys()):
                                    ...
                                    if step_num != 1 and veh_waiting_time_pre.get(aa, {}).get(bb, {}) != {}:
                                        if dd <= veh_position_data_pre[aa][bb] + 10:
                                            ...
                                            intuitive_waiting_time = veh_waiting_time_pre[aa][bb]
                                    if minute_time % 2 == 0 and (int(ee[ee.index(s2) + len(s2):]) - int(
                                            ee[:ee.index(s2) + len(s2)].replace('to', ''))) == 1 and random.randint(0,
                                                                                                                    4) == 2:
            ...
            outfile = open('DQN/ImplicitObservation/Implicit-Observation-DQN-result-OUR-ALGORITHM-run1.csv', 'w')
